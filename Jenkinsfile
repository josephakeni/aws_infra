pipeline{
    ...
    stages{
        ...
        stage("perform terraform apply prod branch "){
            when {
                branch "master"
            }
            steps{
        ...
        stage("perform terraform initialisation & plan dev branch "){
            ...
            steps{
                ...
                withCredentials([string(credentialsId: 'aws_access_key_id', variable: 'AWS_ACCESS_KEY_ID'), string(credentialsId: 'aws_secret_access_key', variable: 'AWS_SECRET_ACCESS_KEY')]) {
                sh '''
                cd ecs/dev/; aws_access_key_id=$AWS_ACCESS_KEY_ID aws_secret_access_key=$AWS_SECRET_ACCESS_KEY terraform init
                aws_access_key_id=$AWS_ACCESS_KEY_ID aws_secret_access_key=$AWS_SECRET_ACCESS_KEY terraform plan -var-file=main.tfvars
                '''
                }
            }
        ...
        stage("perform terraform apply dev branch "){
            ...
            steps{
                ...
                withCredentials([string(credentialsId: 'aws_access_key_id', variable: 'AWS_ACCESS_KEY_ID'), string(credentialsId: 'aws_secret_access_key', variable: 'AWS_SECRET_ACCESS_KEY')]) {
                sh 'cd ecs/dev/; aws_access_key_id=$AWS_ACCESS_KEY_ID aws_secret_access_key=$AWS_SECRET_ACCESS_KEY terraform apply -var-file=main.tfvars --auto-approve'
                }
            }
            post{
                always{
